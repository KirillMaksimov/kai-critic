...
**Ratification is blind by default.** Every checkable claim is verified first; then each finding goes to you one dialog at a time, with the check result as a fact — but **without** the main thread's own verdict and **without** which lens produced it. Both are anchors. Afterwards you see one table: finding · lens · prediction · your ruling · agree? That yields two numbers instead of one — **precision** (your acceptances ÷ findings) and **triage agreement** (how often the main thread predicted your ruling). A low agreement with a high precision means the lenses are fine and the triage is not, which is the more useful failure.
...
`0.1.0`. Single author, extracted from a private vault where it has run for a series of waves on real designs. The charter and the finding format are the settled parts; the ratification protocol is newer and exists precisely because the earlier precision numbers were the main thread triaging objects it had written itself.
